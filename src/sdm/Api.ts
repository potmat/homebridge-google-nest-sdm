...
export class SmartDeviceManagement {
      ...
      this.subscription.on("message", (message) => {
        ...
        if ((event as Events.ResourceEventEvent).resourceUpdate.events) {
          ...
          const device = _.find(
            this.devices,
            (device) =>
              device.getName() === resourceEventEvent.resourceUpdate.name
          );
          if (device) device.event(resourceEventEvent);
          ...
          const device = _.find(
            this.devices,
            (device) =>
              device.getName() === resourceTraitEvent.resourceUpdate.name
          );
          if (device) device.event(resourceTraitEvent);
        }
      });
      this.subscription.on("error", (error) => {
        this.log.error(
          "Plugin initialization failed, there was a failure with event subscription. Did you read the readme: https://github.com/potmat/homebridge-google-nest-sdm#where-do-the-config-values-come-from",
          error
        );
  ...
  async list_devices(): Promise<Device[] | undefined> {
    if (!this.subscribed) return this.devices;

    try {
      const response =
        await this.smartdevicemanagement.enterprises.devices.list({
      ...
      this.log.error(
        "Could not execute device LIST request: ",
