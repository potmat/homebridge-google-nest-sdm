...
export class Camera extends Device {
  ...
  async getSnapshot(): Promise<Buffer | undefined> {
    ...
    if ((await this.getVideoProtocol()) === Traits.ProtocolType.RTSP)
      return await fs.promises.readFile(
        path.join(__dirname, "..", "res", "nest-logo.jpg")
      );
    else
      return await fs.promises.readFile(
        path.join(__dirname, "..", "res", "google-logo.jpg")
      );
  }
  ...
  async getEventImage(eventId: string, date: Date): Promise<void> {
    ...
    if (dateDiff > 30) {
      this.log.debug(
        `Camera event image is too old (${dateDiff} sec), ignoring.`,
        this.getDisplayName()
      );
      return;
    }
    ...
    try {
      const generateResponse = await this.executeCommand<
        Commands.CameraEventImage_GenerateImage,
        Responses.GenerateImage
      >(Commands.Constants.CameraEventImage_GenerateImage, {
        eventId: eventId
      });
      ...
      this.log.error(
        "Could not execute event image GET request: ",
  ...
  async getCameraLiveStream(): Promise<Traits.CameraLiveStream | null> {
    return await this.getTrait<Traits.CameraLiveStream>(
      Traits.Constants.CameraLiveStream
    );
  }

  async getVideoProtocol(): Promise<Traits.ProtocolType> {
    if (
      (await this.getCameraLiveStream())?.supportedProtocols.includes(
        Traits.ProtocolType.WEB_RTC
      )
    ) {
  ...
  async generateStream(
    params?: string
  ): Promise<GenerateRtspStream | GenerateWebRtcStream | undefined> {
    if ((await this.getVideoProtocol()) === Traits.ProtocolType.WEB_RTC) {
      if (!params) throw new Error("Must specify params for WebRTC streams.");
      return this.executeCommand<
        CameraLiveStream_GenerateWebRtcStream,
        GenerateWebRtcStream
      >(Commands.Constants.CameraLiveStream_GenerateWebRtcStream, {
        offerSdp: params
      });
    } else {
      return this.executeCommand<null, GenerateRtspStream>(
        Commands.Constants.CameraLiveStream_GenerateRtspStream
      );
    }
  }
  ...
  async stopStream(id: string): Promise<void> {
    if ((await this.getVideoProtocol()) === Traits.ProtocolType.WEB_RTC) {
      await this.executeCommand<
        Commands.CameraLiveStream_StopWebRtcStream,
        null
      >(Commands.Constants.CameraLiveStream_StopWebRtcStream, {
        mediaSessionId: id
      });
    } else {
      await this.executeCommand<Commands.CameraLiveStream_StopRtspStream, null>(
        Commands.Constants.CameraLiveStream_StopRtspStream,
        {
    ...
    _.forEach(event.resourceUpdate.events, (value, key) => {
      switch (key) {
        ...
        case Events.Constants.CameraPerson:
          if (
            event.eventThreadState &&
            event.eventThreadState != ThreadStateType.STARTED
          )
            return;

          this.getVideoProtocol().then((protocol) => {
            if (protocol === Traits.ProtocolType.WEB_RTC) {
              if (this.onMotion) this.onMotion();
            } else {
              this.getEventImage(
                (value as Events.CameraMotion).eventId,
        ...
        case Events.Constants.CameraSound:
          ...
          this.getVideoProtocol().then((protocol) => {
            if (protocol === Traits.ProtocolType.RTSP) {
              this.getEventImage(
                (value as Events.CameraSound).eventId,
                new Date(event.timestamp)
              );
            }
          });
